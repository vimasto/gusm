...
begin
  ...
  if exists (
    ...
    where iso_weekday = extract(isodow from p_booking_date)::smallint
      and time_block_id = p_time_block_id
  ) then
...
comment on function public.create_booking(smallint, date) is
  'Creates or reactivates a self-service reservation while enforcing closures and admission audit invariants.';
